...
from lichtsensornp import messe # Lichtsensor
...
def Messwerte():  
    ...
    if os.path.exists(verzeichnis+"Messwerte.dat"):    
        ...
        for g in messwerte:             
            # Messwerte auf y-Achse                
            y.append(float(g))     
        # Plot erstellen (mit Beschriftung der Messreihe)                     
        fig,p = plt.subplots(1, 1,num='Lichtmessung')
        # Plot formatieren
        # (k=schwarz o=kleine Punkte, -=Punkte verbinden)
        p.plot(x,y,'ko-')
...
def Hauptprogramm():
    ...
    window.title("Schüler experimentieren - Lichtsensormessung")    
